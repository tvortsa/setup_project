...
async function ensure_config_file() {
    try {
        ...
        if (error instanceof Deno.errors.NotFound) {
            console.log(`Файл конфигурации "${config_file_name}" не найден. Создаю файл с настройками по умолчанию.`);
...
async function remove_project_structure(options) {
    ...
    try {
        ...
        if (error instanceof Deno.errors.NotFound) {
            console.warn(`Файл конфигурации "${config_file_name}" не найден. Нечего удалять.`);
        } else {
...
async function display_project_info() {
    ...
    try {
        ...
        if (error instanceof Deno.errors.NotFound) {
            console.warn(`Файл конфигурации "${config_file_name}" не найден. Информация о проекте недоступна.`);
        } else {
...
async function remove_path(path, dry_run, force) {
    try {
        ...
        if (dry_run) {
            console.log(`[DRY RUN] Would remove: ${path} (type: ${path_stat.isFile ? 'file' : 'directory'})`);
        } else {
            await Deno.remove(path, { recursive: true });
            console.log(`Removed: ${path} (type: ${path_stat.isFile ? 'file' : 'directory'})`);
        }
    } catch (error) {
        if (error instanceof Deno.errors.NotFound) {
